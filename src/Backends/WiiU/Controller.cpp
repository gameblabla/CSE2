...
bool ControllerBackend_GetJoystickStatus(bool **buttons, unsigned int *button_count, short **axes, unsigned int *axis_count)
{
	...
	*axis_count = sizeof(axis_buffer) / sizeof(axis_buffer[0]);
	...
	if (VPADRead(VPAD_CHAN_0, &vpad_status, 1, NULL) == 1)
	{
		...
		button_buffer[7] = vpad_status.hold & VPAD_BUTTON_UP;    // Up
		button_buffer[8] = vpad_status.hold & VPAD_BUTTON_DOWN;  // Down
		button_buffer[9] = vpad_status.hold & VPAD_BUTTON_LEFT;  // Left
		button_buffer[10] = vpad_status.hold & VPAD_BUTTON_RIGHT;// Right
		button_buffer[11] = vpad_status.hold & VPAD_BUTTON_ZL;
		button_buffer[12] = vpad_status.hold & VPAD_BUTTON_ZR;
